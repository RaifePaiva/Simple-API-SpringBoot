...
public class OportunidadeController {
	...
	public ResponseEntity<Oportunidade> buscar(@PathVariable Long id) {
		...
		if(!oportunidade.isPresent()) { // esta verificação s´funciona em versoes do java inferior a 11, isEmpty() nao existe
	...
	public Oportunidade adicionar( @Valid @RequestBody Oportunidade oportunidade) {
		...
		if(oportunidadeExistente.isPresent()) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Já existe uma oportunidade com estas informações");
		}
